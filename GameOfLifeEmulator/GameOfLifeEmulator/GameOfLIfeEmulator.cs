...
namespace GameOfLife
{
    ...
    public class GameOfLIfeEmulator : Game
    {
        ...
        public void UpdateSettings()
        {
            ...
            camera = new Camera(Vector2.Zero, new IntVector2(width, height), settingsManager.LoadedSettings.cameraSpeed);
        ...
        public void LoadGame()
        {
            ...
            switch (settingsManager.LoadedSettings.loadFormat)
            {
                ...
                default:
                    game = LoadRandomGame(new IntVector2(settingsManager.LoadedSettings.randomGameWidth, settingsManager.LoadedSettings.randomGameHeight), settingsManager.LoadedSettings.randomGameProbability, settingsManager.LoadedSettings.destroySideCells);
                    break;
            }
            gameOfLifeScreen = new GameOfLifeScreen(camera, graphics, game, new Vector2(cellSize, cellSize));
        }

        public GameOfLife LoadRandomGame(IntVector2 size, int oneInChance, bool destroySideCells)
        {
        ...
        {
            List<IntVector2> coordsList = new List<IntVector2>();
            for (int x = 0; x < game.SizeX; x++) for (int y = 0; y < game.SizeY; y++)
                    if (game[x, y]) coordsList.Add(new IntVector2(x, y));
            ...
            file.WriteLine("0 0");
            foreach (IntVector2 coords in coordsList) file.WriteLine(string.Format("{0} {1}", coords.X, coords.Y));
            file.Close();
        ...
        public GameOfLife LoadGameFile(string gameFileName, bool destroySideCells)
        {
            ...
            while ((currentLine = file.ReadLine()) != null)
            {
                ...
                xCoords.Add(x);
                yCoords.Add(y);
                coordsList.Add(new IntVector2(x, y));
            }
            file.Close();
        ...
        public GameOfLife LoadCellsFile(string cellsFileName, int cellsPadding, bool destroySideCells)
        {
            ...
            while ((currentLine = file.ReadLine()) != null)
            {
                ...
                {
                    if (currentLine[x] == 'O') coordsList.Add(new IntVector2(cellsPadding + x, cellsPadding + y));
                }
                if (currentLine != "") y++;
            }
            file.Close();
        ...
        public void SetUpGame(GameOfLife game, IntVector2[] coordsList)
        {
            foreach (IntVector2 coords in coordsList) game.ToggleCell(coords.X, coords.Y);
        }
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            for (int x = 0; x < gameOfLifeScreen.gameOfLife.SizeX; x++) for (int y = 0; y < gameOfLifeScreen.gameOfLife.SizeY; y++)
                    gameOfLifeScreen.gameOfLifeBlocks[x, y].colour = gameOfLifeScreen.gameOfLife[x, y] ? Color.Black : Color.White;
    ...
    public class Camera
    {
        ...
        public Camera(Vector2 startPosition, IntVector2 screenSize, float initialVelocity)
        {
            start = startPosition;
    ...
    {
        public ColouredRectangle[,] gameOfLifeBlocks;
        ...
        {
            gameOfLife = game;
            IntVector2 gameOfLifeSize = new IntVector2(gameOfLife.SizeX, gameOfLife.SizeY);
            Texture2D blockTexture = new Texture2D(graphics.GraphicsDevice, (int)blockSize.X, (int)blockSize.Y);
            backgroundColour = Color.Gray;
            gameOfLifeBlocks = new ColouredRectangle[gameOfLifeSize.X, gameOfLifeSize.Y];
            for (int x = 0; x < gameOfLifeSize.X; x++)
            {
                for (int y = 0; y < gameOfLifeSize.Y; y++)
                {
                    Vector2 startCoords = new Vector2(x * blockSize.X, y * blockSize.Y);
                    ColouredRectangle block = new ColouredRectangle(currentCamera, blockTexture, startCoords, Color.White);
                    gameOfLifeBlocks[x, y] = block;
                    AddItem(block);
                }
            }
        }
    ...
    public class IntVector2
    {
        ...
        public IntVector2 FromIntArray(int[] value)
        {
            if (value.Length < 2) throw new ArgumentException("'value' must not have than 2 integers.");
            else return new IntVector2(value[0], value[1]);
        }
        public IntVector2 FromVector2(Vector2 value)
        {
        ...
        {
            return new IntVector2(1);
        }
        public IntVector2 OneX()
        {
            return new IntVector2(1, 0);
        }
        public IntVector2 OneY()
        {
